...
typedef struct {
    enum { login = 0 } messageType;  /* same size as unsigned int */
    unsigned int userID;             /* user identifier */
    unsigned int recipientID;        /* message recipient identifier (0 for login) */
...
typedef struct {
    enum { ackLogin = 0 } messageType; /* same size as unsigned int */
    unsigned int userID;               /* user identifier */
...
_Static_assert(sizeof(((LodiServerToLodiClientAcks *)0)->messageType) == sizeof(unsigned int), "enum size mismatch");
